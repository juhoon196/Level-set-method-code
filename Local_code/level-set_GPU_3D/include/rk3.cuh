/**
 * @file rk3.cuh
 * @brief 3rd-order TVD Runge-Kutta time integration for 3D
 *
 * This module implements the 3-stage 3rd-order TVD Runge-Kutta scheme for
 * time integration of the G-equation in 3D.
 *
...
__constant__ double RK3_ALPHA2_0 = 0.75;
__constant__ double RK3_ALPHA2_1 = 0.25;
__constant__ double RK3_BETA2 = 0.25;

// Stage 3: G^(n+1) = (1/3)*G^n + (2/3)*G^(2) + (2/3)*dt*L(G^(2))
__constant__ double RK3_ALPHA3_0 = 1.0 / 3.0;
__constant__ double RK3_ALPHA3_2 = 2.0 / 3.0;
__constant__ double RK3_BETA3 = 2.0 / 3.0;
...
/**
 * @brief Compute the right-hand side of the G-equation: -u_eff · ∇G (3D)
 *
...
__global__ void computeRHS(const double* __restrict__ G,
    ...
    if (params.s_l > params.epsilon) {
        ...
        dGdx_central = (G[idx(i + 1, j, k, params.nx_total, params.ny_total)] -
                        G[idx(i - 1, j, k, params.nx_total, params.ny_total)]) / (2.0 * params.dx);
        dGdy_central = (G[idx(i, j + 1, k, params.nx_total, params.ny_total)] -
                        G[idx(i, j - 1, k, params.nx_total, params.ny_total)]) / (2.0 * params.dy);
        dGdz_central = (G[idx(i, j, k + 1, params.nx_total, params.ny_total)] -
...
/**
 * @brief Perform one complete RK3 time step (3D)
 *
 ...
 * @param d_u, d_v, d_w Velocity field components (device)
 * @param params Simulation parameters
