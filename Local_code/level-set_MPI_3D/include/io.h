...
inline bool saveFieldBinary3D(const char* filename, const double* local_G,
    ...
    if (params.rank == 0) {
        ...
        // Compute each rank's local_ny and y_start for correct reconstruction
        // (MPI_Gatherv concatenates per-rank blocks, NOT a single contiguous 3D array)
        for (int p = 0; p < params.num_procs; p++) {
            int base_rows = params.ny / params.num_procs;
            int remainder = params.ny % params.num_procs;
            int lny = (p < remainder) ? base_rows + 1 : base_rows;
            int ys  = (p < remainder) ? p * (base_rows + 1)
                                      : remainder * (base_rows + 1) + (p - remainder) * base_rows;
            int offset = displs[p];

            for (int k = 0; k < nz_total; k++) {
                for (int j = 0; j < lny; j++) {
                    for (int i = 0; i < nx_total; i++) {
                        int src = offset + k * nx_total * lny + j * nx_total + i;
                        int dst = k * nx_total * ny_total_g + (ys + j + nghost) * nx_total + i;
