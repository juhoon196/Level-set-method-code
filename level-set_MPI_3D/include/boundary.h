 ...
 * - Y-direction: Zero-gradient at global domain boundaries, MPI exchange for interior
 * - Z-direction: Periodic
 */
inline void applyBoundaryConditions3D(double* G, SimParams& params) {
    // MPI ghost exchange in Y
    ...
    // Zero-gradient at global Y boundaries
    bool is_bottom = (params.neighbor_below == MPI_PROC_NULL);
    bool is_top = (params.neighbor_above == MPI_PROC_NULL);
    applyZeroGradientBC_Y_3D(G, params.nx_total, params.local_ny, params.nz_total,
                              params.nghost, params.local_ny_total, is_bottom, is_top);
}
